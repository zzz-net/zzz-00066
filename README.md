...
- **重复报单拦截（新口径 2026-06）**：**同箱号 + 同原因分类**（**忽略批次号差异**）只要还存在**未关闭**的活跃（待处理/处理中）工单就返回 409 拦截，避免因批次号不同又开一张待处理单；**已关闭后允许重新发起**，作为一条新的独立工单流程（审计日志中以不同的 `ticket_id` 区分前后两次，非同一条流程）
- **撤回后重提重新校验权限**：重提时再次校验 `operator == initiator`，同时检查是否存在重复报单
- **重提后责任归属重置**：转交后被撤回再提交时，`current_handler` 和 `current_handler_role` 重置为发起人及其岗位，不残留上次转交的处理人；代录场景下 `proxy_recorder` 字段保留但不影响责任归属判定
...
| 同箱号同原因重复报单 | 同一异常被反复建单造成管理混乱 | ★ 2026-06 新口径：创建和重提时检查「**box_code + reason_category + 活跃状态**」（忽略批次号差异），关闭后允许重新发起（作为新流程，审计 ID 不同），冲突返回 409 拦截 |
